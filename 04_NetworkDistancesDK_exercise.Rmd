...
date: "23-February-2022 updated `r Sys.Date()`"
output:
...
First I have to solve the measurement task from homework 3 - I have to load cities, inscriptions and required libraries into my environment:
...
```{r, eval=FALSE}
# Wrangle the coordinates into a 2-column format
inscriptions_updated <-inscriptions %>% 
...
Now I can get on to the task!

*Instructions for mean and median distance (assignment 3)*
...
```{r, eval=FALSE}
#Ensuring the cities object has the same CRS as inscriptions object:

st_crs(cities_sf3035) == st_crs(i)

#It is not, so we have to change it to the same crs, and make it into an sf object, so we can continue! 


# Transform the projection to a 2D projection using EPSG 3035

insc_sf3035 <- st_as_sf(i, coords = c("longitude", "latitude"), crs = 3035)


#Checking again, to see that it is now the same:

st_crs(cities_sf3035) == st_crs(insc_sf3035)

# It is! Now we can create a convex hull around the inscriptions's points dissolved into a MULTIPOINT
insc_ch <- st_convex_hull(st_union(insc_sf3035))

# Select cities that fall within the convex hull of the inscriptions

reduced_cities <- st_intersection(insc_ch, st_transform(cities_sf3035))
...
```{r, eval=FALSE}
#identify the coordinates of Aros:

Aros <- mb_geocode("Aros, Aros Allé 2, Aarhus, 8000, Denmark ")
Aros

#using setview to make a map centering Aros:
mapbox_map %>% 
  setView(lng = Aros[1],
          lat = Aros[2], 
          zoom = 14)
...
```{r directions-moesgaard, eval=FALSE}
...
All the places that are nok marked with either yellow or red - this is the smaller towns that lay between the bigger cities, where the hospitals are primarily located (obviously). I don't know if there's a way to calculate the total area, but from looking at the visuals I would say maybe around 25% of the total area of Midtjylland.  
...
When trying to change the time, almost all of the area of Midtjylland was covered within 30 minutes - however there is a small area on Djursland that is not covered at this point, and might have to use 40 minutes (I also tried with 35, which still didn't cover the outliers). 
...
no_cars <- st_read("../data/no_cars.csv", header = FALSE)
...
#loading the data
hh <- st_read("../data/households2020_2021.csv", header = FALSE)


#changing the names of the columns
names(hh) <- c("Household type","municipality","hh2020", "hh2021")


#grouping by municipality and calculating the mean of the households 2021 (I had problems making it work with both 2020, and 2021, so I decided to go with just 2021 as I didn't have enough time for wrangling more)
 
                                                   
hh_mun <- hh %>% group_by(municipality) %>% summarize(sum(as.numeric(hh2021)))
                                                 
                                                                                    
#changing the columnnames
names(hh_mun) <- c("municipality","Households_2021")

#joining the dataframe of no cars and households by municipality
hh_nocar <- no_cars %>% inner_join(hh_mun, by = "municipality")

#using mutate to create a new column with the percentage of households with no cars in 2021

hh_nocars <- hh_nocar %>%mutate(Percentage2021=paste0((as.numeric(nc2021)/Households_2021)*100))



#reading in the population data - I couldn't get the st_read function to work here, so i made a workaround
munic <- read.csv("../data/munic_pop_2022Q1.csv", header = FALSE)


#using slice to filter out the two first rows in the dataset, as they had messy data
pop <- munic %>% slice(-c(1,2))

#changing the column names 

names(pop) <- c("municipality","population")

#joining the nocars dataframe to the population dataset by municipality
no_cars_pct_mun <- pop %>% inner_join(hh_nocars, by = "municipality")

#using mutate to create a new column, containing the average household for each municipality 

pop_pet_hh <- no_cars_pct_mun %>% mutate(as.numeric(population)/as.numeric(Households_2021))
...
#sorting the kommunes with 25% or more households with no car
kommuner_less_cars <- no_cars_pct_mun %>% filter(Percentage2021 >= 25)

#changing the place names into latitude and longitude coordinates:
kommuner_less_cars_st <- oc_forward_df(placename = kommuner_less_cars$municipality, countrycode = "DK", limit =1, no_annotations = TRUE)

#changing into an sf object

kommuner_less_cars_sf <- st_as_sf(kommuner_less_cars_st, coords = c("oc_lng", "oc_lat"), crs = 4326)
...
leaflet() %>% 
  addTiles() %>% 
  addMarkers(kommuner_less_cars_st$oc_lng, kommuner_less_cars_st$oc_lat, popup = kommuner_less_cars_st$oc_formatted)
...
If my calculations are right (which I'm not sure they are, cause it seems like a lot), the areas that have 25 and more percent of carless houesholds are kommunerne: 

Skive, Viborg, Randers, Norddjurs, Holstebro, Ikast, Herning, Silkeborg, Aarhus, Odder, Horsens, Samsø. 
...
```{r risk-areas-sol, eval=FALSE}

library(sf)


#kommuner_less_cars_sf %>% st_transform(4326)



#mapping the maps of kommuner with 25% or more households with no cars and the areas with more than 20 minutes walking or biking distance to hospitals to compare. I couldn't make the st_difference function work, so I made a more manual work-around 
...
Yes, Ikast, Norddjurs, Struer. Not sure what their total area is though.


*4. How would you ensure residents in these areas have reasonable access to hospitals?*

Maybe by having more easy accesible public transport. 
