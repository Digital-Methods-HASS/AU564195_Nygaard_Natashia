...
date: "15/02/2022 updated `r format(Sys.time(), '%B %d, %Y')`" 
output:
...
# Verify the projection is 'projected' not 'geographic' - using the crs function to find information. This shows, that it is projected. 
crs(cities_sf3035)
```
...
The measurement units is meters
...
```{r buff, eval=FALSE}
#running the st_crs function on the object, to inspect the measurement units - I can see that it is meters. 
...
* Check the column names for something that looks like spatial data, either Lat/Long, X/Y or a `coordinates` column.
  ...
  Looks like a degrees (lat, long), so an unprojected CRS
  
  - If their meaning is clear, are they in the format you need them to be for an easy conversion to an sf object?
  Yes, but i need to seperate the longitude and latitude into seperate columns and clean them, removing everything but numbers. 
...
download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json","/work/cds-spatial/inscriptions.json")

# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("/work/cds-spatial/inscriptions.json")
...
leaflet() %>% addTiles() %>% addMarkers(lng=inscriptions_updated$longitude,lat=inscriptions_updated$latitude)
```
...
The inscriptions are (mostly) from Italy!
...
Cause it shows the area with inscriptions, and measures in degrees, which is the values we have in the dataset.
...
Yes, seems about right - looks like Italy - however it is in slightly different position than what we are used to seeing. 
